...
5. In each round of rib_process(), the rnh's resolving route status will be checked. The backwalk for fast convergence stops if it has ROUTE_ENTRY_NHG_ID_PRESERVED set or the NHT list is empty, then the status for ROUTE_ENTRY_NHG_ID_PRESERVED is also cleared
