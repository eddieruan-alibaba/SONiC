...
4. zebra_rnh_eval_dependents() traverses backward to locate the parent nhe that depends on the current nhe. It then updates the parent nhe's dependency tree with the new one and proceeds to perform a nexthop refresh for the parent nhe.
